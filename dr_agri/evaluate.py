...
Neural models (MLP, LSTM, GRU)
    ...
    validation partition drives early stopping. The test partition is used
    exactly once, to produce the reported score.
...
def run_neural(data: CropData, view, dr_seconds: float, sizes, dr_name: str,
               model_name: str, seed: int, seq_len: int) -> Result:
    (X_tr, y_tr), (X_va, y_va), (X_te, y_te) = view["train"], view["val"], view["test"]
    if model_name == "MLP" and X_tr.shape[1] != 1:
        X_tr, X_va, X_te = X_tr[:, -1:, :], X_va[:, -1:, :], X_te[:, -1:, :]

    l1 = 0.0
    if model_name == "LSTM" and data.task == "classification":
        l1 = 1e-4 if dr_name == "No Reduction" else (2e-4 if dr_name in {"PCA", "KernelPCA", "BOTCAST"} else 0.0)
    max_epochs = 20 if model_name == "MLP" and data.task == "regression" else 100
    if model_name == "LSTM" and (data.task == "regression" or dr_name in {"Isomap", "MDS"}):
        max_epochs = 200
    model = NeuralModel(model_name, data.task, input_dim=X_tr.shape[2], dr_name=dr_name,
                        seed=seed, l1=l1, max_epochs=max_epochs)
    t0 = time.perf_counter()
    ...
    return Result(crop=data.crop, task=data.task, dr=dr_name, model=model_name, seed=seed,
                  score=float(_score(data.task, y_te, y_hat)), n_components=int(X_tr.shape[2]),
                  fit_seconds=fit_s, predict_seconds=predict_s,
                  n_train=sizes[0], n_test=sizes[2],
                  seq_len=1 if model_name == "MLP" else seq_len,
